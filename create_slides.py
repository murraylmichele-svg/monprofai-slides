...
---DIAPOSITIVE 1---
TYPE: TITRE
TITRE: Le troc et les routes commerciales
CONTENU:
- Comprendre ce qu'est le troc et son importance
- Découvrir la célèbre Route de la Soie
NOTE_ENSEIGNANT: Présenter les objectifs clairement.
---FIN DIAPOSITIVE 1---
"""
...
def draw_angle(requests, slide_id, idx, angle_deg, label, ox, oy, length=120):
    ...
    requests.append({'createLine': {
        ...
        'elementProperties': {
            ...
            'size': {'width': {'magnitude': emu(length), 'unit': 'EMU'}, 'height': {'magnitude': 1, 'unit': 'EMU'}},
            'transform': {'scaleX': 1, 'scaleY': 1, 'translateX': emu(ox), 'translateY': emu(oy), 'unit': 'EMU'}}}})
    ...
    requests.append({'createLine': {
        ...
        'elementProperties': {
            'pageObjectId': slide_id,
            'size': {'width': {'magnitude': emu(arm_w), 'unit': 'EMU'}, 'height': {'magnitude': emu(arm_h), 'unit': 'EMU'}},
            'transform': {'scaleX': 1, 'scaleY': 1, 'translateX': emu(min(ox, x2)), 'translateY': emu(min(oy, y2)), 'unit': 'EMU'}}}})
    requests.append({'createShape': {
        'objectId': label_id, 'shapeType': 'TEXT_BOX',
        'elementProperties': {
            'pageObjectId': slide_id,
            'size': {'width': {'magnitude': emu(100), 'unit': 'EMU'}, 'height': {'magnitude': emu(25), 'unit': 'EMU'}},
            'transform': {'scaleX': 1, 'scaleY': 1, 'translateX': emu(ox), 'translateY': emu(oy + 10), 'unit': 'EMU'}}}})
    requests.append({'insertText': {'objectId': label_id, 'text': label}})
    requests.append({'updateTextStyle': {
        'objectId': label_id,
        'style': {'fontFamily': 'Arial', 'fontSize': {'magnitude': 11, 'unit': 'PT'}, 'bold': True,
...
def create_presentation(slide_data, title):
    ...
    pres_file = drive_service.files().create(
        body=file_metadata,
        fields='id',
        supportsAllDrives=True
    ).execute()
    ...
    for i, slide in enumerate(slide_data):
        ...
        requests.append({'createShape': {
            ...
            'elementProperties': {
                'pageObjectId': slide_id,
                'size': {'width': {'magnitude': emu(680), 'unit': 'EMU'}, 'height': {'magnitude': emu(55), 'unit': 'EMU'}},
                'transform': {'scaleX': 1, 'scaleY': 1, 'translateX': emu(20), 'translateY': emu(12), 'unit': 'EMU'}}}})
        ...
        if is_defis:
            ...
            for col_id, col_x, col_text in cols:
                requests.append({'createShape': {
                    ...
                    'elementProperties': {
                        'pageObjectId': slide_id,
                        'size': {'width': {'magnitude': col_width, 'unit': 'EMU'}, 'height': {'magnitude': col_height, 'unit': 'EMU'}},
                        'transform': {'scaleX': 1, 'scaleY': 1, 'translateX': col_x, 'translateY': col_top, 'unit': 'EMU'}}}})
                requests.append({'insertText': {'objectId': col_id, 'text': col_text}})
                requests.append({'updateTextStyle': {
                    'objectId': col_id,
                    'style': {
                        'fontFamily': 'Arial', 'fontSize': {'magnitude': 14, 'unit': 'PT'}, 'bold': False,
                        'foregroundColor': {'opaqueColor': {'rgbColor': {'red': 0, 'green': 0, 'blue': 0}}}},
                    'fields': 'fontFamily,fontSize,bold,foregroundColor'}})
        else:
            ...
            if slide['contenu']:
                ...
                requests.append({'createShape': {
                    ...
                    'elementProperties': {
                        'pageObjectId': slide_id,
                        'size': {'width': {'magnitude': emu(content_width), 'unit': 'EMU'}, 'height': {'magnitude': emu(270), 'unit': 'EMU'}},
                        'transform': {'scaleX': 1, 'scaleY': 1, 'translateX': emu(20), 'translateY': emu(80), 'unit': 'EMU'}}}})
                requests.append({'insertText': {'objectId': contenu_id, 'text': slide['contenu']}})
                requests.append({'updateTextStyle': {
                    'objectId': contenu_id,
                    'style': {
                        'fontFamily': 'Arial', 'fontSize': {'magnitude': font_size, 'unit': 'PT'}, 'bold': False,
                        'foregroundColor': {'opaqueColor': {'rgbColor': {'red': 0, 'green': 0, 'blue': 0}}}},
                    'fields': 'fontFamily,fontSize,bold,foregroundColor'}})
    ...
    for i, slide in enumerate(slide_data):
        if slide.get('note'):
            ...
            notes_requests.append({'insertText': {'objectId': notes_page_id, 'text': slide['note']}})
    if notes_requests:
        slides_service.presentations().batchUpdate(
            presentationId=pres_id,
